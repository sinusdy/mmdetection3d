...
from .drone_data_utils import get_drone_info
...
def create_drone_info_file(data_path,
                           pkl_prefix='drone',
                           save_path=None,
                           relative_path=True):
    """Create info file of Drone dataset.
    ...
    drone_infos_train = get_drone_info(
        data_path,
        training=True,
        velodyne=True,
        image_ids=train_img_ids,
        relative_path=relative_path)
    #_calculate_num_points_in_gt(data_path, drone_infos_train, relative_path)
    filename = save_path / f'{pkl_prefix}_infos_train.pkl'
    print(f'Drone info train file is saved to {filename}')
    mmcv.dump(drone_infos_train, filename)
    drone_infos_val = get_drone_info(
        data_path,
        training=True,
        velodyne=True,
        image_ids=val_img_ids,
        relative_path=relative_path)
    #_calculate_num_points_in_gt(data_path, drone_infos_val, relative_path)
    filename = save_path / f'{pkl_prefix}_infos_val.pkl'
    print(f'Drone info val file is saved to {filename}')
    mmcv.dump(drone_infos_val, filename)
    filename = save_path / f'{pkl_prefix}_infos_trainval.pkl'
    print(f'Drone info trainval file is saved to {filename}')
    mmcv.dump(drone_infos_train + drone_infos_val, filename)

    drone_infos_test = get_drone_info(
        data_path,
        training=False,
        label_info=False,
        velodyne=True,
    ...
    print(f'Drone info test file is saved to {filename}')
    mmcv.dump(drone_infos_test, filename)
